...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      _this.$ajax.post('http://127.0.0.1:9000/business/admin/sms/save', _this.sms).then((response) => {
        let resp = response.data;
        if (resp.success) {
    ...
    del(id) {
      ...
      Confirm.show("删除短信验证码后不可恢复，确认删除？", function () {
        _this.$ajax.delete('http://127.0.0.1:9000/business/admin/sms/delete/' + id).then((response)=>{
          let resp = response.data;
          if (resp.success) {
            _this.list(1);
            Toast.success("删除成功！");
