...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      _this.$ajax.post('http://127.0.0.1:9000/system/admin/role/save', _this.role).then((response) => {
        let resp = response.data;
        if (resp.success) {
    ...
    listRoleUser(){
      let _this = this;
      _this.roleUsers = [];
      _this.$ajax.get('http://127.0.0.1:9000/system/admin/role/list-user/' + _this.role.id).then((response) => {
        let resp = response.data;
        let userIds = resp.content;

        //根据加载到的用户ID，到【所有用户数组：users】中查找用户对象，用于列表显示
