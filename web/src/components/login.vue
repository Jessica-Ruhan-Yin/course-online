          ...
          <div class="forget-div" v-show="MODAL_STATUS === STATUS_FORGET">
            ...
            <div class="form-group">
              <input v-on:blur="onForgetMobileBlur()"
                     ...
                     class="form-control" placeholder="手机号">
              <span v-show="forgetMobileValidate===false" class="text-danger">手机号11位数，且必须已注册</span>
            </div>
            <div class="form-group">
              <div class="input-group">
...
export default {
  ...
  methods: {
    ...
    register() {
      ...
      // 调服务端注册接口
      _this.$ajax.post('http://127.0.0.1:9000/business/web/member/register', _this.memberRegister).then((response) => {
    ...
    login() {
      ...
      _this.$ajax.post('http://127.0.0.1:9000/business/web/member/login', _this.member).then((response) => {
        let resp = response.data;
        if (resp.success) {
    ...
    sendSmsForForget() {
      ...
      _this.$ajax.get('http://127.0.0.1:9000/business/web/member/is-mobile-exist/' + _this.memberForget.mobile).then((response) => {
        let resp = response.data;
        if (resp.success) {
    ...
    resetPassword() {
      ...
      // 调服务端密码重置接口
      _this.$ajax.post('http://127.0.0.1:9000/business/web/member/reset-password', _this.memberForget).then((res) => {
